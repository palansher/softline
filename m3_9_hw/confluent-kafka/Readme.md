...
- [Задание 3.9 - Запись и чтение объектов в/из Kafka (confluent-kafka)](#задание-39---запись-и-чтение-объектов-виз-kafka-confluent-kafka)
  ...
  - [Особенности реализации](#особенности-реализации)
    - [kafka module](#kafka-module)
      - [Основные отличия в коде](#основные-отличия-в-коде)
...
Точечное наименование ключей конфига (используются точки вместо нижних подчеркиваний, например bootstrap.servers).

Метод отправки называется .produce(), а результаты доставки обрабатываются через колбэк on_delivery.

В продюсере необходим регулярный вызов .poll(0) для обработки фоновых событий и вызова колбэков.
