...
def pass_data(request):
    ...
    if age:
        if age.isdigit():
            age = int(age)
            return HttpResponse(
                f"Добрый день, {name}, Ваш год рождения: {datetime.datetime.now().year - age}"
            )
...
def demo_parse_data_in_template(request):
    ...
    item = {"title": "Ауди", "price": 1000, "color": "Белый"}

    cars = [
        {"title": "Ауди", "price": 1000, "color": "Белый"},
        {"title": "БМВ", "price": 1200, "color": "Черый"},
        {"title": "VW", "price": 900, "color": "Серый"},
    ]

    return render(
        request, "demo_parse.html", {"cities": cities, "item": item, "cars": cars}
    )
...
def get_cars(request):
    ...
    return HttpResponse(
        f"max_price: {max_price}, sum_price: {sum_price}, count: {count}"
    )
...
def create_query(request):
    # Показать автомобили, которые стоят больше тысячи.
    cars = Car.objects.raw(
        "SELECT * FROM my_app_car WHERE price > %s ORDER BY id DESC", [1000]
    )
    s = ""
    for car in cars:
        s += f"{car.name}: {car.price}<hr>"
...
def demo_delete(request):
    return HttpResponse(Car.objects.filter(id=3).delete())


def get_car(request):
    car_id = request.GET.get("id")

    # 1. Проверяем, что ID передан и состоит только из цифр
    if car_id and car_id.isdigit():
        try:
            # 2. Используем get() вместо filter(), чтобы получить один объект
            car = Car.objects.get(id=int(car_id))
            return HttpResponse(car.name)
        except Car.DoesNotExist:
            return HttpResponse("Автомобиль с таким ID не найден!")

    return HttpResponse("ID не передан или неверный формат!")
