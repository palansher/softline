...
def generate_trantion_data():
    ...
    timestamp = datetime.datetime.now().strftime("%d.%m.%Y %H:%M:%S")  # noqa: DTZ005
...
def delivery_callback(err, msg):
    if err:
        ...
        logger.info(
            f"Сообщение отправлено в топик {msg.topic()},"
            f"Партиция: {msg.partition()},"
            f"Ключ сообщения: {msg.key()},"
            f"Тело сообщения: {msg.value().decode('utf-8')}"
        )
...
def send_tranctions():
    ...
    logger.info(
        f"Дата отправки: {datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S')}"  # noqa: DTZ005
    )
    ...
    try:
        ...
        logger.info("Все транзакции успешно выполнены!")
...
def get_info():
    """
    - сделать объект класса Consumer
    - Подписаться на топик
    - Ждать, когда в нем появится сообщения
    """
    ...
    try:
        ...
        while True:
            ...
            if msg.error():
                ...
                continue
            count = count + 1
            key = msg.key().decode("utf-8")  # pyright: ignore[reportOptionalMemberAccess]
            value = msg.value().decode("utf-8")  # pyright: ignore[reportOptionalMemberAccess]
...
# В одной функции и работа продюсера, и работа консюмера.
def main():
    print("=" * 50)
