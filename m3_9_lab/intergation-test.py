...
# Тест на то, что в контейнере создается таблица.
# чтобы работать с контейнером Docker
import docker
...
# Делаем потомка unittest.TestCase
class MyTestIntegrationForCrudTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        """
        Данный метод запускается до всех тестов, т.е. первым
        чтобы до тестов были подняты контейнеры.
        """
        # Создаем клиент-docker, используя переменный среды.
        cls.client = docker.from_env()
        # Просим клиента запустить нам контейнер
        ...
        time.sleep(10) # ждем, пока контейнер запустится- 10 секунд на то, чтобы все поднялось

        # Получение динамического порта контейнера
        cls.container.reload() # обновляем информацию о контейнере
